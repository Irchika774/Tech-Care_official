...
import { Search, Star, MapPin, Briefcase, CheckCircle, DollarSign, Phone, Mail, Monitor, Wrench, SearchX, Navigation, Filter, TrendingUp, Map as MapIcon, List, Cpu } from 'lucide-react';
...
const PCRepair = () => {
    ...
    try {
      ...
      if (techData && techData.length > 0) {
        ...
        const transformed = techData.map(tech => ({
          ...
          specialization: tech.services || tech.specialization || ['PC Repair', 'Laptop Repair'],
          ...
          priceRange: tech.price_range || { min: 1000, max: 25000 },
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtimeService.subscribeToTechnicians((payload) => {
      console.log('[PCRepair] Real-time update:', payload.eventType);
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            toast({
              ...
              description: "Showing all available PC technicians.",
            });
            fetchAllTechnicians();
          }
        );
  ...
  const loadFromGoogleSheets = async () => {
    try {
      ...
      const techData = shops
        .filter(shop => shop.services?.some(s => s.toLowerCase().includes('pc') || s.toLowerCase().includes('laptop')))
        .map(shop => ({
          _id: shop.id,
          id: shop.id,
          name: shop.name,
          specialization: shop.services || ['PC Repair'],
  ...
  return (
    <div className="bg-black text-white">
      <SEO
        title="PC & Laptop Repair - TechCare"
        description="Professional computer and laptop repair technicians near you. Hardware upgrade, screen replacement, and more."
      />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-4 mb-8">
              ...
              <span className="text-sm tracking-[0.3em] uppercase text-gray-400">PC Repair</span>
              <div className="w-3 h-3 bg-white"></div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-12">
            ...
            <h2 className="text-3xl font-bold text-white">
              PC Repair Experts ({filteredTechnicians.length})
            </h2>
          </div>
          ...
          {loading && technicians.length === 0 ? (
            ...
            <div className="text-center py-24 border border-white/10">
              ...
              <p className="text-gray-400">No technicians found for these filters.</p>
            </div>
          ) : (
...
const TechnicianCard = ({
  ...
  const priceMin = technician.priceRange?.min || 1000;
  const priceMax = technician.priceRange?.max || 25000;
...
const DetailModal = ({ technician, convertPrice, isFavorite, onToggleFavorite, onSchedule, userLocation, calculateDistance }) => {
  ...
  const priceDisplay = `${convertPrice(technician.priceRange?.min || 1000)} - ${convertPrice(technician.priceRange?.max || 25000)}`;
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div>
          <img src={technician.profileImage} className="w-full aspect-square object-cover mb-4" />
          <p className="text-gray-400 leading-relaxed">{technician.description || 'Expert hardware and software repair services for all laptop and PC brands.'}</p>
        </div>
